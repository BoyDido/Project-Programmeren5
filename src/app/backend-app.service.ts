...
export class BackendAppService {
  ...
  /* GET notes whose user or content contains search term */
  searchNotes(term: string, name: string): Observable<Note[]> {
    console.log(term);
    if (!term.trim()) {
      // if not search term, return empty notes array.
      return of([]);
    }
    return this.http.get<Note[]>(`${this.usersUrl}/notesSearch?term=${term}&name=${name}`)
      .pipe(tap(x => {
        x.length ?
